...
for (const el of await page.$$(cards)) {
  const offer =
    (await el.evaluate(
      (e) => e.querySelector("picture + div > div")?.textContent
    )) || "";
...
console.log(
  `${restaurants.length} potential restaurants with offers found! closing puppeteer...`
);
await browser.close();
...
for (let i = 0; i < restaurants.length; i++) {
  ...
  const rawData = JSON.parse(
    decodeURIComponent(JSON.parse(`"${reactData.trim()}"`))
  );
  ...
  if (section.isOnSale && data.catalogSectionsMap[section.uuid]) {
    ...
    if (deals.length) {
      ...
      console.log(
        `got data for ${compiled.name}: ${deals.length} deal(s) found`
      );
    } else {
